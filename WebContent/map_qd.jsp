...
<jsp:useBean id="manager_qd" scope="session" class="com.stanley.TaiWeiManager_QD" />

<%
JSONRPCBridge.registerObject("manager_qd", manager_qd);
%>
...
function getLatestTaiWeis() {
	...
	jsonrpc.manager_qd.getLatestTaiWeis(cbSet, set);
}
...
function refreshMarker(_map){ 
	if(refreshTimes<=2){
	//alert("refreshTimes="+refreshTimes);
	refreshTimes++;
	getLatestTaiWeis();
	_map.clearOverlays();
//----------------------------
...
function AmoyMapType() {
	...
	this.getSrc = function(level, row, column) {
		return "${root}/images/qd/zoom_"+level+"/"+level+"-"+row+"-"+column+".jpeg"; 
	}
}
