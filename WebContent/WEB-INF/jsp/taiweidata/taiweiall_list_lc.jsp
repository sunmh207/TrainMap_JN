...
<body>
<html:topNav/>
<form id="searchForm" action="${root}/taiweialldata_lc.do">
	...
	<tr>
		...
		<tr>
			...
			<s:if test="currentCH==null">
				<a href=${root}/taiweialldatainput_lc!input.do?gdName=<s:property value="gdName"/>&CH=<s:property value="currentCH"/>>+添加机车</a>
			</s:if>
			<s:else>
				<a href=${root}/taiweialldata_lc!deleteLoco.do?gdName=<s:property value="gdName"/>&CH=<s:property value="currentCH"/> onclick="return fDelCheck()">-移除机车</a>&nbsp;&nbsp;
				<a href=${root}/taiweialldatainput_lc!forMove.do?gdName=<s:property value="gdName"/>&CH=<s:property value="currentCH"/> >->移动机车</a>
			</s:else>
			</td>
		</tr>
	</s:iterator>
