...
<form id="searchForm" action="${root}/youhuoyubao.do">
...
<input type="hidden" name="qryLocalOnly" value='${qryLocalOnly}' >
	...
	<input type="button" onclick="location.href='${root}/youhuoyubaoinput!input.do'" value="新增">
</div>
...
<table width="100%" class="table" align="center" cellpadding="1" cellspacing="0" bgcolor="#FFFFFF">
	...
	<c:forEach items="${objectList}" var="obj" varStatus="status">
		<tr>
			...
			<!--  "${defaultInfoType}" -->
			<c:if test="${obj.infoType==defaultInfoType}">
			<a href="${root}/youhuoyubaoinput!input.do?youhuoyubao.id=${obj.id}">编辑</a>&nbsp;&nbsp;
			<a href="${root}/youhuoyubao!delete.do?youhuoyubao.id=${obj.id}" onclick="return fDelCheck()">删除</a>&nbsp;&nbsp;
			</c:if>
			</td>
		</tr>
	</c:forEach>
