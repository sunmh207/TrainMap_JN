...
import com.stanley.TaiWeiManager_LC;
import com.stanley.locomotivedict.LocomotiveService;

public class TaiWeiAllDataAction_LC extends JITActionBase implements Preparable {
	private TaiWeiManager_LC svr_lc;
	...
	public void prepare() throws JTException {
		...
		if (gdName != null&& !"".equals(gdName)) {
			gudao = svr_lc.findGudaoByGDName(gdName);
		}
	}
	public String forMove() {
		try{
			List<Object> emptyTaiweis= svr_lc.queryByHql("from TaiWeiAll where currentCH is null and didian='聊城' order by gdName");
	...
	public String input(){
		try{
			//locoList=locoSvr.queryLocalLocoList("聊城");
	...
	public String save() throws Exception {
		try {
			if(gudao!=null){
				...
				svr_lc.updateBo(gudao);
				
				jicheYunxingInfo = svr_lc.findJicheYunxingInfoByCH(CH);
				if(jicheYunxingInfo==null){
					jicheYunxingInfo=new JicheYunxingInfo();
					jicheYunxingInfo.setCH(CH);
					svr_lc.createBo(jicheYunxingInfo);
				}
			}
		} catch (Exception e) {
	...
	public String move() throws Exception {
		try {
			Gudao oldGudao = svr_lc.findGudaoByGDName(gdName);
			if(oldGudao!=null){
				oldGudao.setCH(null);
				oldGudao.setLastUpdate(DateUtil.getCurrentTime());
				svr_lc.updateBo(oldGudao);
			
			}
			Gudao newGudao = svr_lc.findGudaoByGDName(newGDName);
			if(newGudao!=null){
					newGudao.setCH(CH);
					newGudao.setLastUpdate(DateUtil.getCurrentTime());
					svr_lc.updateBo(newGudao);
